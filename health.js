...
// Mapea métodos si el módulo existe, si no, stubs
const {
  initialize,
...
export async function hcGetStatusDebug() {
  if (Platform.OS !== 'android' || !HC) return { status: -1, label: 'NOT_AVAILABLE' };
  try {
    await ensureInit();
...
// ---- Lecturas (con stubs si no hay HC) ----
export async function readTodaySteps() {
  if (Platform.OS !== 'android' || !HC) return { steps: 0, source: 'na', origins: [], asOf: null };
  await ensureInit();
...
export async function readLatestHeartRate() {
  if (Platform.OS !== 'android' || !HC) return { bpm: null, at: null, origin: null };
  await ensureInit();
  try {
    const end = new Date();
...
export async function readLatestSpO2() {
  if (Platform.OS !== 'android' || !HC) return { spo2: null, at: null, origin: null };
  await ensureInit();
  try {
    const end = new Date();
    const start = new Date(end.getTime() - 1000 * 60 * 60 * 48);
...
export async function readSleepLast24h() {
  if (Platform.OS !== 'android' || !HC) return { hours: null, origins: [], rangeEnd: null };
  await ensureInit();
  try {
    const end = new Date();
